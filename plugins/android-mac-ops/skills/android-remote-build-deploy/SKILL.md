...
- Source mounting: use `android-source-access`.
- Framework diagnosis/verification: use `android-framework-change-workflow`.

## Inputs

From `android-source-access` registry or project `.codex`:
...
- `scripts/resolve-remote-mapping.sh`: resolve `SSH_HOST` and `REMOTE_ROOT` from `android-source-access` registry.
...
- `android-source-access`: source mounting, remount recovery, and project mapping registry.
