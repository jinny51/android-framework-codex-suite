...
5. `android-remote-channel` 提供稳定远程会话，避免重复 SSH、重复 tmux、重复锁逻辑。
6. 本插件的 `android-remote-build-deploy` 负责服务器构建、产物定位和本地设备推送。
7. `android-framework-change-workflow` 根据需求和设备证据给最终验收结论，并决定 `validated`、`candidate`、`draft`、`failed` 或 `blocked` 包状态（package status）。
8. `android-framework-patch-capture` 把已完成、阶段性、失败或阻塞但有价值的 Framework 功能整理成一个功能 README、多个源码仓库 patch 和 evidence。
...
- 本插件负责核心工程不变量、Android Framework 证据链和知识系统。
- 平台插件只负责源码接入；构建交付在本插件中共用一套实现。
- 当个人规范和本插件流程都适用时，Codex 应同时满足个人规范，并保留本插件的源码证据、构建证据、设备验证证据和风险说明。
- `jinny-framework-coding-standards` 只是旧入口兼容 wrapper；规范权威在本插件的 `android-change-policy`。
