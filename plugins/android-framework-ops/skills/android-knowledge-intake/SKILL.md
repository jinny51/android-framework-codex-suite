...
If a daily package contains multiple candidates that share one base model, such as `TVE1067M1` and `TVE1067M1_H031`, write the shared base model and keep the complete candidates in `project_inference`. If candidates have different base models, keep `project=unknown` and preserve the ambiguity.
