...
Daily and weekly report bodies are the primary human-readable product. Generate `reports/daily.md` and `reports/weekly.md` with the Android Framework work-report template, including project overview, concrete items, handling method, result, remaining issues, next steps, risks, outputs, weekly project statistics, patch output, verification/delivery, and next-week plan. Also write `materials/display/report_view.json` as a UI read model for cards, lists, and detail panes; it is a structured index of the same report, not a separate AI/evidence layer. Keep `work_findings.json` as evidence for audit and later analysis.
