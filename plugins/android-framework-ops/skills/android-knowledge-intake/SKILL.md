...
description: Use when handling member first setup, doctor checks, plugin updates, config migration, daily/weekly/patch compatibility commands, or the shared member-side incoming kernel. Prefer the split daily, weekly, or patch intake skills for new business package generation. Do not use for knowledge curation decisions.
---

# Android Knowledge Intake

Use this skill for member-side incoming shared kernel work, setup, doctor checks, plugin updates, config migration, and compatibility command entry. New member-facing business entrypoints are split into `android-daily-report-intake`, `android-weekly-report-intake`, and `android-framework-patch-intake`; they all route to this shared script and protocol layer instead of copying upload logic.
...
Daily and weekly report bodies are the primary human-readable product. Generate `reports/daily.md` and `reports/weekly.md` with the Codex office report templates. Daily reports answer 今天干了什么、怎么干的、结果是什么. Weekly reports answer 本周完成多少、还剩多少、风险和依赖是什么、下周怎么收敛. They must summarize by project blocks based on the weekly范文: 项目名称、客户名称、接到文档时间、来源说明、需求类型、需求结构、本周完成、当前剩余、预计完成, then 项目详情 and 下周计划. Do not use a large overview table in the weekly `本周概况`, and do not repeat daily execution logs. Also write `materials/display/report_view.json` as a UI read model for cards, lists, and member report detail; it is a structured index of the same report, not a separate AI/evidence layer. `report_view.json` uses `schema=akbs-report-view-human-v1`; `material_name` is project + customer, `material_summary` is the daily topic or weekly completed/remaining/risk summary, and project details live under `projects[]`. Do not emit deprecated report fields such as `display_title`, `ui_card`, `one_line_summary`, `project_ledgers`, `weekly_progress_summary`, or `weekly_detail_sections`. Management-side aggregation may consume the same fields later, but the member-side flow must not ask members to understand team summary concepts. Keep `work_findings.json` as evidence for audit and later analysis.
