...
$candidatePaths = @(
    (Join-Path $env:USERPROFILE ".codex\skills\android-wsl-remote-channel\scripts\Invoke-AndroidRemoteChannel.ps1"),
    (Join-Path $PSScriptRoot "..\..\android-wsl-remote-channel\scripts\Invoke-AndroidRemoteChannel.ps1")
)

$channelScript = $candidatePaths | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
if (-not $channelScript) {
    Write-Error "android-wsl-remote-channel is not installed or not found. Expected one of: $($candidatePaths -join '; ')"
    exit 127
}
