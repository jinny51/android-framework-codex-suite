...
- First-time CIFS/Samba mounting or reboot remount recovery. Use `android-source-access`.
...
2. Exact local-project mapping from `android-source-access` registry.
...
Do not compare local and remote path names as a path-consistency check. The mounted local path may intentionally differ from the remote path because `android-source-access` can correct platform and project naming from source evidence. Validate the handoff mapping instead.
...
Resolve `LOCAL_REPO` first and confirm it looks like an Android source tree. If the path is missing or unusable, stop and use `android-source-access`.
...
If the mapping is missing, use `android-source-access` recovery or listing as the next diagnostic path; do not rederive remote paths from local path segments. If repo `.codex` and mount registry disagree, surface the conflict and prefer the exact mount registry unless the user or caller explicitly overrides it.
...
If this reports `TMUX_MISSING`, run the channel's explicit install action. It will try passwordless sudo, `CODEX_REMOTE_SUDO_PASSWORD`, then saved `android-source-access` credentials; if none works, ask the user for the remote sudo password and rerun with that env var set.
...
- `local-source-missing`: stop and use `android-source-access`.
...
- `scripts/resolve-remote-mapping.sh`: resolve `SSH_HOST` and `REMOTE_ROOT` from `android-source-access` registry for a mounted local project.
...
- `android-source-access`: source mounting, remount recovery, and exact local-to-remote mapping registry.
