...
- `android-windows-source-access`: owns SMB/UNC source mapping and registry.
- `android-windows-remote-build-deploy`: owns Android build/deploy semantics and local `adb.exe` delivery.
...
```powershell
$ChannelDir = "$env:USERPROFILE\.codex\skills\android-windows-remote-channel"
...
Use `install-tmux` only as an explicit action. It tries passwordless sudo, `CODEX_REMOTE_SUDO_PASSWORD`, then credential candidates saved by `android-windows-source-access`. It never writes credentials.
...
- `REMOTE_ROOT_MISSING`: resolve the source mapping again through `android-windows-source-access`.
