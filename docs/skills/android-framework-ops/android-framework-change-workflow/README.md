...
当工作产生可复用、可参考、失败或阻塞经验时，应通过 `android-framework-patch-capture` 整理补丁资料，再由 `android-framework-patch-intake` 生成 incoming 包。`android-knowledge-intake` 只保留共享内核和兼容命令入口。
