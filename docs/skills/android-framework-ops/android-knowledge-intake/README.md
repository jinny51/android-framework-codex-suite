...
该 skill 用于在成员本机自动汇总 Codex 会话、源码改动记录、patch、readme 和验证结果，先生成本地 `pending`（待检查包），再提交到数据库仓库的 `incoming` 目录。

成员端 Codex 是知识生成主体。它负责从会话、git、patch 和验证记录里整理上传包；数据库仓库服务器收到 `incoming` 后只做确定性验收和审核队列，不直接批准为 AI 知识。AI 知识由管理员审核和导出流程写入知识库仓库。
...
- 成员在检查窗口内补充或修正内容；到点后自动提交到数据库仓库 `incoming`。
- Framework 修改满足条件时，成员端 Codex 自动通过 `patch-capture -> intake` 生成补丁包 incoming。
...
如果这个补丁包明确来自某个 `daily_trace` 或 `weekly_trace` incoming 包，显式带上 run id，后续审核/导出链路会用它做确定性关联：
...
- [config.example.toml](../../../../plugins/android-framework-ops/skills/android-knowledge-intake/config.example.toml)：成员本机配置示例，使用数据库仓库/知识库仓库字段。
