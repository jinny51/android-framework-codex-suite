...
成员周报的核心是项目总盘子：项目名称、来源类型（定制 / Buglist / 混合 / 临时支持）、来源说明、启动时间、已持续时间、新增功能、移植适配、Bug、其他、合计、本周完成、累计完成、当前剩余、预计完成周和风险。生成结果允许成员在上传前手动修正；`project_ledgers[]` 是成员个人周报的结构化读模型，供管理端后续汇总使用，成员端不暴露团队汇总概念。
